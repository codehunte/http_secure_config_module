...
2. limit_conn
3. limit_rate
4. white_black_list_conf, white_list, black_list
...
where是指配置作用位置。。 该值是个数字，取值来源于http://you host/sec_config?for_each 返回结果中的index字段的数据。
...
示例7：
curl -d "method=add&cmd_name=limit_rate_conf&arg=www.talkyun.com 1M" http://your host/manager/secure_config_update
增加 限制host信息，限制www.talkyun.com 的带宽为1M byte

示例8：
curl -d "method=del&cmd_name=limit_rate_conf&arg=www.talkyun.com" http://your host/manager/secure_config_update
删除 限制host信息，删除对www.talkyun.com 的带宽限制
